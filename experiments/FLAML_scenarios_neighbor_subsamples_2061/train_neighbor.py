...
## train model
train = urban_LE[(np.abs(urban_LE["lat"]-known_gridcell[p]["lat"])<0.0001) & 
                 (np.abs(urban_LE["lon"]-known_gridcell[p]["lon"])<0.0001)]
feature_ls = fd["CAM"]
automl = AutoML()
automl_settings = {
...
with open(parquet_save_path+p+"_"+start_year + "_" + end_year + "_neighbor_subsamples.pkl", 'wb') as f:
